...
==============================================================================
* Modified by Lin Yi (https://github.com/linyiLYi/pose-monitor)
* Further modified by Pine Style in 2026
*/
...
class MoveNet(private val interpreter : Interpreter, private var gpuDelegate : GpuDelegate?) : PoseDetector {
            ...
            when (device) {
                ...
                Device.GPU -> {
                    try {
                        ...
                        android.util.Log.w("MoveNet", "GPU不支持，回退到CPU模式: ${e.message}")
                        gpuDelegate = null
                    }
                }
                ...
                Device.NNAPI -> {
                    try {
                        ...
                        android.util.Log.i("MoveNet", "NNAPI模式已启用")
                    }
                    catch (e : Throwable) {
                        android.util.Log.w("MoveNet", "NNAPI不支持，回退到CPU模式: ${e.message}")
                    }
                }
            }
            try {
                return MoveNet(
                    Interpreter(FileUtil.loadMappedFile(context,
                        // if (modelType == ModelType.Lightning) LIGHTNING_FILENAME
                        // else THUNDER_FILENAME
                        THUNDER_FILENAME), options), gpuDelegate)
            }
            catch (e : Throwable) {
                android.util.Log.w("MoveNet", "使用设备 $device 创建Interpreter失败，尝试回退到CPU模式: ${e.message}")
                val cpuOptions = Interpreter.Options()
                cpuOptions.setNumThreads(CPU_NUM_THREADS)
                return MoveNet(
                    Interpreter(FileUtil.loadMappedFile(context, THUNDER_FILENAME), cpuOptions), null)
            }
        }
    ...
    override fun estimatePoses(bitmap : Bitmap) : List<Person> {
        ...
        cropRegion?.run {
            val rect =
                RectF((left * bitmap.width), (top * bitmap.height), (right * bitmap.width), (bottom * bitmap.height))
    ...
    private fun torsoVisible(keyPoints : List<KeyPoint>) : Boolean {
        return ((keyPoints[BodyPart.LEFT_HIP.position].score > MIN_CROP_KEYPOINT_SCORE).or(
            keyPoints[BodyPart.RIGHT_HIP.position].score > MIN_CROP_KEYPOINT_SCORE)).and(
    ...
    private fun determineRectF(
        ...
        keyPoints.forEach {
            targetKeyPoints.add(KeyPoint(it.bodyPart, PointF(it.coordinate.x, it.coordinate.y), it.score))
        }
        if (torsoVisible(keyPoints)) {
            val centerX =
                (targetKeyPoints[BodyPart.LEFT_HIP.position].coordinate.x + targetKeyPoints[BodyPart.RIGHT_HIP.position].coordinate.x) / 2f
            val centerY =
                (targetKeyPoints[BodyPart.LEFT_HIP.position].coordinate.y + targetKeyPoints[BodyPart.RIGHT_HIP.position].coordinate.y) / 2f
            
            val torsoAndBodyDistances = determineTorsoAndBodyDistances(keyPoints, targetKeyPoints, centerX, centerY)
            
            val list = listOf(torsoAndBodyDistances.maxTorsoXDistance * TORSO_EXPANSION_RATIO,
    ...
    private fun determineTorsoAndBodyDistances(
        ...
        val torsoJoints =
            listOf(BodyPart.LEFT_SHOULDER.position, BodyPart.RIGHT_SHOULDER.position, BodyPart.LEFT_HIP.position,
        ...
        return TorsoAndBodyDistance(maxTorsoYRange, maxTorsoXRange, maxBodyYRange, maxBodyXRange)
    }
}
